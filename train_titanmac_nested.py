...
def train_titanmac_nested(
    ...
    reset_memory_after_eval: bool = False,
):
    """
    ...
    Args:
        ...
        reset_memory_after_eval: Reset neural memory after each eval to prevent
                                 eval data from contaminating training
    ...
    while step < config.max_steps:
        for batch_idx, batch in enumerate(train_loader):
            ...
            if step % config.eval_every == 0 and step > 0:
                ...
                # Reset neural memory after eval to prevent eval data contamination
                if reset_memory_after_eval:
                    model.reset_memory()
                    print("  [Memory Reset] Neural memory cleared after eval")
...
def main():
    ...
    parser.add_argument("--reset-memory", action="store_true",
                        help="Reset neural memory after each eval to prevent eval data contamination")
    ...
    model, metrics, history = train_titanmac_nested(
        ...
        reset_memory_after_eval=args.reset_memory,
    )
